...
createDayLogFile (*Coll, *Sub, *Name, *Res, *LPath, *Lfile, *L_FD) {
# Create a log sub-directory within *Coll if it is missing
# Create a dat timestamped log file with the input file name *Name
  msiGetSystemTime(*Time,"unix");
  *TimeH = timestrf(datetime(double(*Time)), "%Y-%m-%d");

  *Head = *Name;
  *End = "";
  *out = errormsg(msiSplitPathByKey (*Name, ".", *Head, *End), *Msg);
  if (*End == "") { *Vers = *Head ++ "." ++ "*TimeH"; }
  else {  *Vers = *Head ++ "." ++ "*TimeH." ++ *End; }
...
# uploadFiles: input string * input list string * input string -> integer
    ...
    *cs = getCollections(*fs);
    ...
    for(*i=0;*i<size(*fs);*i=*i+1) {
        *obj = elem(*fs,*i);
        msiDataObjPut("*coll/*obj", "dfc-defaultResc", "localPath=*lf++++forceFlag=", *status);
    }
}
